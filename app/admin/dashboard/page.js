...
const Dashboard = async () => {
  ...
  const mockStocks = {
    data: [
      { _id: '1', name: 'Laptop Computer', quantity: 10, uom: 'pcs', sellingPrice: 50000 },
      { _id: '2', name: 'Wireless Mouse', quantity: 25, uom: 'pcs', sellingPrice: 1500 },
      { _id: '3', name: 'USB Cable', quantity: 50, uom: 'pcs', sellingPrice: 300 },
      { _id: '4', name: 'Keyboard', quantity: 15, uom: 'pcs', sellingPrice: 2500 },
      { _id: '5', name: 'Monitor', quantity: 8, uom: 'pcs', sellingPrice: 25000 }
    ]
  };

  const mockPurchaseSales = {
    data: {
  ...
  let categories, stocks, res;
  
  try {
    [categories, stocks, res] = await Promise.all([
      getCategories(6).catch(() => mockCategories),
      getStocks(5).catch(() => mockStocks),
      getPurchaseAndSales().catch(() => mockPurchaseSales),
    ]);
  } catch (error) {
    console.log('Using mock data for dashboard');
    categories = mockCategories;
    stocks = mockStocks;
    res = mockPurchaseSales;
  }
                ...
                {stocks?.data?.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-3 font-medium text-black">
                      {item?.quantity || 0} - {item?.uom || 'pcs'}
                    </td>
                    <td className="px-2 py-3 text-black">
                      {item?.sellingPrice || 0} Tk.
                    </td>
                  </tr>
                ))}
              </tbody>
